...
void PatchEstimator::imageCB(const sensor_msgs::Image::ConstPtr& msg)
{
	...
	if (depthEstimators.size() > 0)
	{
		...
		for (uint16_t ii = 0; ii < depthEstimators.size(); ii++)
		{
			...
			Eigen::Vector3f pciHatICLExt = depthEstimators.at(ii)->uc;
			Eigen::Vector3f pciHatEKF = depthEstimators.at(ii)->mc;
			pciHatICLExt *= depthEstimators.at(ii)->dcHatICLExt;
...
void PatchEstimator::update(std::vector<cv::Point2f>& pPts, std::vector<cv::Point2f>& kPts, std::vector<cv::Point2f>& cPts, Eigen::Vector3f vc, Eigen::Vector3f wc, ros::Time t, float dt)
{
