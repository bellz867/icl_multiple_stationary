...
bool PatchEstimator::initialize(cv::Mat& image, nav_msgs::Odometry imageOdom, ros::Time t)
{
	...
	cv::cornerSubPix(image,ptCorners,cv::Size(3,3),cv::Size(-1,-1),cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 30, 0.1));
...
void PatchEstimator::match(cv::Mat& image, float dt, Eigen::Vector3f vc, Eigen::Vector3f wc, ros::Time t, Eigen::Vector3f ppcHat, Eigen::Vector4f qpcHat, Eigen::Vector3f pkc, Eigen::Vector4f qkc)
{
	...
	for (int ii = 0; ii < flows.size(); ii++)
	{
		...
		std::cout << " flowx " << flows.at(ii).x << " flowy " << flows.at(ii).y << " chiVal " << chiTestVal << " badCount " << depthEstimators.at(ii)->badCount << std::endl;
	...
	// if (normGood && angGood)
	if ((qAng < 45.0) && (2*currentBound.width > (numberFeaturesPerPartCol-1)*minDistance/2) && (2*currentBound.height > (numberFeaturesPerPartRow-1)*minDistance/2) && (depthEstimators.size() > minFeaturesBad))
	{
		try
		{
				...
				if (allPtsKnown)
				{
					for (int ii = 0; ii < cPts.size(); ii++)
					{
						...
						for (int jj = 0; jj < qkcs.size(); jj++)
						{
							...
							// std::cout << " " << cPti3Proj(0) << " " << cPti3Proj(1) << " " << cPti3Proj(2);
							...
							DcPti3(1) = cPti3Proj(2) - cPts3.at(ii).z;
